...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("lycancoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("lycancoin://", Qt::CaseInsensitive))
    {
...
{
    QString ret = QString("lycancoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    if (GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Lycancoin (testnet).lnk";
    else if (GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Lycancoin (regtest).lnk";    
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (GetBoolArg("-testnet", false))
            optionFile << "Name=Lycancoin (testnet)\n";
        else if (GetBoolArg("-regtest", false))
            optionFile << "Name=Lycancoin (regtest)\n";
        else
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the lycancoin app
