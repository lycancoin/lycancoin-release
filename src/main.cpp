...
CFeeRate CTransaction::minTxFee = CFeeRate(100000000);  // Override with -mintxfee
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
/*** Minimum lycancoin transaction set to 1 ***/
CFeeRate CTransaction::minRelayTxFee = CFeeRate(100000000);
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (block.vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees))
        return state.DoS(100,
                         error("ConnectBlock() : coinbase pays too much (actual=%dvs limit=%d)",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // if 12960 of the last 17,280 blocks are version 2 or greater (last 30 days on Lycancoin network) (51/100 if testnet):
    if (block.nVersion >= 2 && 
