...
uint256 hashGenesisBlock("0x50f80e3dea383a355eb15e4be1f122acbc4144bceaa86604555953c5b8a0c9e4"); //1391485370
...
const string strMessageMagic = "Lycancoin Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 4500 LYC if made of 50 LYC inputs)
            if (nBytes < 10000)
                nMinFee = 0;
        }
        else
...
{
    int64 nSubsidy = 2970 * COIN;

	// Subsidy is cut in half every 800000 blocks, which will occur approximately every 4.5 years
    nSubsidy >>= (nHeight / 800000); // lycancoin: 800k blocks in ~3.8 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 60 * 60; // Lycancoin: 2 hour
static const int64 nTargetSpacing = 150; // Lycancoin: 2.5 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Lycancoin miner
...
bool CBlock::AcceptBlock()
{
    ...
    // Reject block.nVersion=1 blocks when 95% of the last 30 days (17,280 blocks) (75% on testnet ~ not maintained on Lycancoin network) of the network has upgraded:
    if (nVersion < 2)
    {
        if ((!fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 16416, 17280)) ||
    ...
    if (nVersion >= 2)
    {
        // if 12,960 of the last 17,280 blocks are version 2 or greater (last 30 days on Lycancoin network) (51/100 if testnet):
        if ((!fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 12960, 17280)) ||
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Lycancoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1391485370; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 6678936;

        if (fTestNet)
        {
            block.nTime    = 1389323189;
            block.nNonce   = 1730203;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x269910b6413f0b424d62db021fed2758ce6761f9b45f5e3a7640ef9dfbe2c218"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Lycancoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
...
{
    printf("LycancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("lycancoin-miner");
    ...
    while (fGenerateBitcoins)
    {
        ...
        printf("Running LycancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
